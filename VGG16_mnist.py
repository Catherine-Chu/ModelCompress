...
class PrunningFineTuner_VGG16:
    ...
    def __init__(self, train_path, test_path, model, use_gpu, torch_version = 0.4):
        ...
        if use_gpu:
            ...
            pin_memory=False
        self.train_data_loader = dataset.loader(train_path,pin_memory=pin_memory)
        self.test_data_loader = dataset.test_loader(test_path,pin_memory=pin_memory)
    ...
    def prune(self):
        ...
        for it_k in range(iterations):
            ...
            torch.save(model,"./pruned/vgg16_mnist_prunned_%d" % it_k)
        ...
        torch.save(model, "vgg16_mnist_prunned")
...
def get_args():
    ...
    parser.add_argument("--train_path", type=str, default="./mnist_data")
    parser.add_argument("--test_path", type=str, default="./mnist_data")
...
if __name__ == '__main__':
    ...
    with open('./vgg_out_mnist.txt', 'w+', 1) as redirect_out:
        ...
        if use_gpu:
            if args.train:
                ...
                if args.restore:
                    model.load_state_dict(torch.load("vgg16_model_mnist").state_dict())
            elif args.prune:
            ...
            if args.train:
                ...
                model = torch.load("vgg16_model_mnist")
        ...
        if args.train:
            ...
            torch.save(model, "vgg16_model_mnist")
